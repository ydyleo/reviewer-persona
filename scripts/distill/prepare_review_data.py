...
# ── 代码模式聚类规则表（来源：legacy prepare_review_data.py，保持不变）──
# (pattern_label, category, code_keywords(regex), comment_keywords, target_code_patterns, desc)
# category: S=规范, P=性能, B=业务逻辑, A=架构, C=并发, D=可读性
PATTERN_RULES = [
    ("const/auto&缺失", "S",
     [r'\bauto\b'],
...
def classify_review(target_code, comment, file_path):
    ...
    matched = []
    for label, category, _code_keywords, comment_keywords, regexes, _desc in PATTERN_RULES:
        comment_hit = any(kw.lower() in comment_lower for kw in comment_keywords)
        code_hit = any(re.search(rx, target_code) for rx in regexes)
        if comment_hit or code_hit:
            matched.append((label, category))
    if not matched:
        return [('其他', 'D')]
    return matched
